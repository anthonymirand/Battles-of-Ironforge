...
public class CrossbowCheck : MonoBehaviour {
	...
	void Update () {
		other = GameObject.FindGameObjectWithTag("Wizard");
		if (other != null && (Vector3.Distance (other.transform.position, self.transform.position) < distance)) {
			particles.enableEmission = true;
			if (DragonHandler.DragonFound && CrossbowHandler.CrossbowFound && Time.time > delay){
